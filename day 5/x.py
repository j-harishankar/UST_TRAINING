...
def on_save():
    ...
    if emp_id.strip() and age.isdigit():
        ...
        messagebox.showwarning('Value Error', 'EmpID is mandatory and Age must be numeric')
...
tk.Label(page1_frame, text="Employee Type").pack()
emp_type_var = tk.StringVar(value="Regular")
emp_type_dropdown = ttk.Combobox(page1_frame, textvariable=emp_type_var,
                                 values=["Regular", "Fixed Time", "Intern"],
                                 state="readonly")
emp_type_dropdown.pack(pady=10)

tk.Button(page1_frame, text="Save", command=on_save).pack(pady=10)
tk.Button(page1_frame, text="View Records", command=on_view_records).pack(pady=10)
...
tk.Label(page2_frame, text="Filter by Employee Type").pack()
filter_dropdown = ttk.Combobox(page2_frame, textvariable=filter_var,
                               values=["All", "Regular", "Fixed Time", "Intern"],
                               state="readonly")
